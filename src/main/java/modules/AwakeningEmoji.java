...
public enum AwakeningEmoji {
    ...
    private static HashMap<Integer, GuildEmoji> awakeningEmojis;
    ...
    public static void loadEmojis(ArrayList<Guild> servers) {
        awakeningEmojis = new HashMap<>();
        for (int i = 0; i < servers.size(); i++) {
            Guild g = servers.get(i);
            AwakeningEmoji[] vals = AwakeningEmoji.values();
            for (int j = 0; j < vals.length; j++) {
                GuildEmoji e = getEmojiByName(g, vals[j].emote);
                if (e != null)
    ...
    public static GuildEmoji getEmoji(int id) {
        if (awakeningEmojis.containsKey(id))
            return awakeningEmojis.get(id);
        else return awakeningEmojis.get(-1);
    }

    private static GuildEmoji getEmojiByName(Guild guild, String name) {
        List<GuildEmoji> emotes = guild.getEmojis().collectList().block();
        for (GuildEmoji e : emotes) {
